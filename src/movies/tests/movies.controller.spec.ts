...
describe("MoviesController", () => {
  ...
  it("should find all movies", async () => {
    ...
    expect(await moviesController.findAll(new FindAllQueryDto())).toEqual(result);
  ...
  it("should find one movie", async () => {
    ...
    expect(await moviesController.findOne("1")).toEqual(movies[0]);
  ...
  it("should not find one movie", async () => {
    ...
    await expect(moviesController.findOne("1")).rejects.toThrow(NotFoundException);
    expect(prismaMock.movie.findUnique).toHaveBeenCalledTimes(1);
  });
  ...
  it("should create movie", async () => {
    ...
    prismaMock.movie.findUnique.mockResolvedValue(null);
    expect(await moviesController.create(movies[0])).toEqual(movies[0]);
  });

  it("should not create movie with conflicting id", async () => {
    prismaMock.movie.create.mockResolvedValue(null);
    prismaMock.movie.findUnique.mockResolvedValue(movies[0]);

    await expect(moviesController.create(wrongMovieData)).rejects.toThrow(ConflictException);
  });
  ...
  it("should not create movie with wrong data", async () => {
    ...
    await expect(moviesController.create({} as CreateMovieDto)).rejects.toThrow(
      BadRequestException,
    );
  ...
  it("should delete movie", async () => {
    ...
    expect(await moviesController.delete("1")).toEqual(movies[0]);
    expect(prismaMock.movie.findUnique).toHaveBeenCalledTimes(1);
  ...
  it("should not delete movie", async () => {
    ...
    prismaMock.movie.findUnique.mockResolvedValue(null);
    await expect(moviesController.delete("1")).rejects.toThrow(NotFoundException);
  ...
  it("should edit movie", async () => {
    ...
    expect(await moviesController.edit("1", movies[0])).toEqual(movies[0]);
    expect(prismaMock.movie.findUnique).toHaveBeenCalledTimes(1);
    expect(prismaMock.movie.update).toHaveBeenCalledTimes(1);
  });

  it("should not edit movie", async () => {
    prismaMock.movie.update.mockResolvedValue(null);
    prismaMock.movie.findUnique.mockResolvedValue(null);

    await expect(moviesController.edit("0", wrongMovieData as EditMovieDto)).rejects.toThrow(
      NotFoundException,
    );
    expect(prismaMock.movie.findUnique).toHaveBeenCalledTimes(1);
